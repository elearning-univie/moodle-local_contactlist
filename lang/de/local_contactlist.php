<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * Strings for component 'local_contactlist', language 'de'.
 *
 * @package       local_contactlist
 * @author        Angela Baier
 * @copyright     2020 University of Vienna
 * @since         Moodle 3.8+
 * @license       http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

$string['email'] = 'Email address';
$string['errornotallowedonpage'] = 'Sie sind nicht in diesem Kurs angemeldet!';
$string['errorlistnotshown'] = 'Dieser Kurs bietet keine Kontaktliste an.';
$string['chat'] = 'Nachrichten';
$string['changegloballink'] = '&nbsp;&nbsp;&nbsp;<a href="{$a->here}"> Ändern sie die moodleweite Kontaktlistensichtbarkeit HIER.</a>';
$string['customcoursefieldlabel'] = 'Kontaktlistensichtbarkeit in der Kursnavigation';
$string['globaldefaultsetting'] = 'Verwende mooldeweite Kontaktlistensichtbarkeit (default).<br><a href="{$a->here}"> Ändern sie die moodleweite Kontaktlistensichtbarkeit HIER.</a>';
$string['invisible'] = 'Unsichtbar';
$string['localvisible'] = 'Sie sind derzeit SICHTBAR in diesem Kurs!';
$string['localinvisible'] = 'Sie sind derzeit UNSICHTBAR in diesem Kurs!';
$string['localvisibility'] = 'Kontaktinformationen (Name, Email, Profilbild und Chat) Sichtbarkeit in diesem Kurs.';
$string['localvisibility_help'] = 'Info';
$string['modalwarning'] = 'Sie sind derzeit nicht in der Kontaktliste dieses Kurses sichtbar. Wollen sie ihre Kontaktinformationen (Name, Profilbild, E-mail Adresse und Chat) mit allen Kursteilnehmern teilen?';
$string['name'] = 'Vorname';
$string['nodename'] = 'Kontaktliste';
$string['pagetitle'] = 'Kontaktliste';
$string['pluginname'] = 'Kontaktliste';
$string['privacy:metadata:local_contactlist_course_vis'] = 'Information über Kontaktlistensichtbarkeit für einen bestimmten Kurs. Diese Information kann sich von der moodleweiten Einstellung unterscheiden.';
$string['privacy:metadata:local_contactlist_course_vis:courseid'] = 'Die ID des Kurses in welchem der User (un)sichtbar ist.';
$string['privacy:metadata:local_contactlist_course_vis:userid'] = 'Die ID des Users, welcher (un)sichtbar ist in einem bestimmten Kurs.';
$string['privacy:metadata:local_contactlist_course_vis:visib'] = 'Die Sichtbarkeitseinstellung für einen bestimmten Kurs.';
$string['privacy:metadata:user_info_data'] = 'Informationen über Die Kontaktlistensichtbarkeit für die ganze moodle Instanz. Default ist unsichtbar.';
$string['privacy:metadata:user_info_data:data'] = 'Sichtbarkeit für die ganze moodle Instanz.';
$string['privacy:metadata:user_info_data:userid'] = 'Die UserID.';
$string['role'] = 'Rolle';
$string['surname'] = 'Familienname';
$string['totalvsvisible'] = '<p><b>{$a->visible} von {$a->total} in diesem Kurs angemeldeten Teilnehmer teilen Ihre Kontaktiformation.</b></p>';
$string['userprofilelabel'] = 'Kontaktlistensichtbarkeit für alle Kurse.<br>Teilen der Kontaktiformation (Name, Profilbild, Email-Adresse, Chat) in allen angemeldeten Kursen.';
$string['visible'] = 'Sichtbar';
$string['visib_help'] = 'Kontaktlistensichtbarkeit für diesen Kurs. Sollten sie keinen Wert auswählen, wird die moodleweite Einstellung aus den Privatsphäreeinstellungen des Profils verwendet. Wenn sie ihre Kontaktiformationen teilen, ist ihr Name, Profilbild, E-mail Adresse und Chat für alle Kursteilnehmer sichtbar.';
